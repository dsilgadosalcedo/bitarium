...
const gridFieldClass = `h-full w-full rounded-none border border-[var(--sign-in-grid-line)] bg-[var(--sign-in-canvas-bg)] px-3 text-base text-[var(--sign-in-card-text)] shadow-none outline-none ${SIGN_IN_FIELD_TRANSITION} placeholder:text-[var(--sign-in-card-muted)] focus-visible:border-[var(--draw-purple-foreground)] focus-visible:ring-0`

const gridButtonClass =
  "flex h-full w-full cursor-pointer items-center justify-center rounded-none border border-[var(--sign-in-button-border)] bg-[var(--sign-in-button-bg)] text-base font-semibold text-[var(--sign-in-button-text)] shadow-none transition-colors hover:bg-[var(--sign-in-button-hover)] focus-visible:border-[var(--draw-purple-foreground)] focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50"
